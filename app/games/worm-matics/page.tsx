...
export default function WormMaticsGame() {
  const [gameState, setGameState] = useState<"LOBBY" | "TUTORIAL" | "PLAYING" | "GAMEOVER" | "HALL_OF_FAME">("LOBBY");
  ...
  useEffect(() => {
    if (gameState === "PLAYING") {
      ...
      // 시간에 따라 속도가 빨라지는 백그라운드 정기 스폰 (초반 8초 시작, 최소 3초)
      const scheduleNextSpawn = () => {
        let currentPlayTime = 0;
        setPlayTime((t) => { currentPlayTime = t; return t; });
        const delay = Math.max(3000, 8000 - currentPlayTime * 20 + Math.random() * 2000);
  ...
  // 지렁이가 0마리가 되면 강제로 하나 생성
  useEffect(() => {
    if (gameState === "PLAYING" && worms.length === 0 && playTime > 2) {
      setTimeout(triggerSpawn, 500);
    }
  }, [worms, gameState, triggerSpawn, playTime]);
  ...
  const handlePointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (gameState === "TUTORIAL" && tutorialWorm) {
      ...
      if (prevHead.x !== gridX || prevHead.y !== gridY) {
        ...
        setTutorialWorm({ ...tutorialWorm, segments: updatedSegs });
      }
    } else if (gameState === "PLAYING") {
      setWorms((prevWorms) =>
        prevWorms.map((w) => {
      ...
      if (activeWorm) {
        ...
        if (targetHole) {
          if (targetHole.value === activeWorm.equation.answer) {
            ...
            // 첫 리젠은 약 3초 전후로 시작, 너무 빠르지 않게 가속
            const nextSpawnDelay1 = Math.max(1500, 3000 - playTime * 10 + Math.random() * 500);
            const nextSpawnDelay2 = Math.max(2500, 4500 - playTime * 15 + Math.random() * 1000);
  ...
  const resetTutorialWorm = () => {
    ...
    setTutorialWorm((prev) => {
      if (!prev) return null;
      return { ...prev, isDragging: false, segments: [{ x: 5, y: 7 }, { x: 4, y: 7 }, { x: 3, y: 7 }] };
    });
  };
  ...
  const resetActiveWorm = (worm: Worm) => {
    ...
    setWorms((prev) =>
      prev.map((w) => {
        ...
        return { ...w, isDragging: false, segments: newSegs };
      })
    );
  };
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/math_worm_scores`, {
        ...
        body: JSON.stringify({
          ...
          nickname: "Player", // 스키마 조건 우회
          score: score,
        }),
      });
      if (!response.ok) throw new Error("Score insert failed");
      fetchRankings();
    } catch (e) {
      console.error(e);
      saveScoreLocally();
  ...
  useEffect(() => {
    if (gameState === "GAMEOVER" || gameState === "HALL_OF_FAME") {
      fetchRankings();
    }
  }, [rankTab, gameState]);
  ...
  return (
    <div className="flex-1 w-full h-full max-w-4xl mx-auto p-4 flex flex-col justify-center items-center overflow-hidden font-sans">
      ...
      {gameState === "LOBBY" && (
        <div className="w-full max-w-md bg-white text-black p-8 retro-border bg-slate-50 dark:bg-slate-900 dark:text-white rounded-lg mt-8 shadow-xl">
          <Link href="/games" className="text-sm text-cyan-500 mb-4 inline-block hover:underline">
            ← 수학 게임 목록으로
          </Link>
          <h2 className="text-4xl text-center mb-6 text-green-500 font-bold retro-text-shadow">
            🐛 Worm-matics 🐛
          </h2>
          <p className="text-sm text-center mb-8 text-slate-500 dark:text-slate-400">
            반과 번호를 선택하고 지렁이를 정답 구멍에 빠르게 넣어주세요!
          </p>
          ...
          <form onSubmit={handleLobbySubmit} className="space-y-6">
            ...
            <button 
              type="button" 
              onClick={() => { playSound("click"); setGameState("HALL_OF_FAME"); }} 
              className="w-full retro-btn retro-btn-success py-3 text-md mt-2"
            >
              🏆 명예의 전당 보기
            </button>
          </form>
      ...
      {gameState === "HALL_OF_FAME" && (
        <div className="w-full max-w-2xl bg-white text-black dark:bg-slate-900 dark:text-white p-6 retro-border rounded-lg flex flex-col items-center">
          <h2 className="text-3xl text-center text-yellow-500 font-bold mb-6 retro-text-shadow">
            🏆 명예의 전당 🏆
          </h2>

          <div className="flex w-full max-w-md border-b-4 border-black mb-4">
            <button
              onClick={() => { playSound("click"); setRankTab("ALL"); }}
              className={`flex-1 py-3 text-center font-bold text-sm ${rankTab === "ALL" ? "bg-yellow-300 text-black" : "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-400"}`}
            >
              전체 랭킹
            </button>
            <button
              onClick={() => { playSound("click"); setRankTab("CLASS"); }}
              className={`flex-1 py-3 text-center font-bold text-sm ${rankTab === "CLASS" ? "bg-yellow-300 text-black" : "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-400"}`}
            >
              {className}반 랭킹
            </button>
          </div>

          <div className="w-full max-h-72 overflow-y-auto border-4 border-black bg-slate-950 p-2 mb-6 rounded">
              ...
              <table className="w-full text-left font-retro text-xs border-collapse">
                ...
                <tbody>
                  {rankingList.map((record, index) => (
                    <tr key={index} className="border-b border-slate-800 hover:bg-slate-900 text-white">
                      <td className="py-2 px-2">{index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}`}</td>
                      <td className="py-2 px-2">{record.class_name}#{String(record.number).padStart(2, "0")}</td>
          ...
          <button onClick={() => { playSound("click"); setGameState("LOBBY"); }} className="retro-btn w-full max-w-md py-3 text-sm">
            뒤로 가기
          </button>
        </div>
      )}
      ...
      {gameState === "TUTORIAL" && (
        <div className="w-full bg-white dark:bg-slate-900 text-black dark:text-white p-4 retro-border rounded-lg flex flex-col items-center">
          <h3 className="text-lg font-bold mb-2 text-center text-yellow-500">튜토리얼</h3>
          ...
          <div className="w-full text-center">
            {tutorialStep === 0 ? (
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <button onClick={() => { playSound("click"); setGameState("PLAYING"); }} className="retro-btn retro-btn-success py-2 px-6 animate-pulse">
                  본 게임 시작하기
                </button>
              </div>
            )}
          ...
          <div className="w-full flex flex-wrap justify-between items-center bg-black text-white p-2 border-b-4 border-black mb-2 font-retro select-none text-[10px] sm:text-xs gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="w-20 sm:w-32 bg-slate-800 border-2 border-black h-3 sm:h-4 flex overflow-hidden">
                <div
                  className={`h-full transition-all duration-300 ${worms.length > 8 ? "bg-red-500" : worms.length > 5 ? "bg-yellow-500" : "bg-emerald-500"}`}
      ...
      {gameState === "GAMEOVER" && (
        <div className="w-full max-w-2xl bg-white text-black dark:bg-slate-900 dark:text-white p-6 retro-border rounded-lg flex flex-col items-center overflow-hidden">
          ...
          <div className="flex w-full max-w-md border-b-4 border-black mb-4">
            <button
              onClick={() => { playSound("click"); setRankTab("ALL"); }}
              className={`flex-1 py-3 text-center font-bold text-xs sm:text-sm ${rankTab === "ALL" ? "bg-yellow-300 text-black font-black" : "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-400"}`}
            >
              전체 랭킹
            </button>
            <button
              onClick={() => { playSound("click"); setRankTab("CLASS"); }}
              className={`flex-1 py-3 text-center font-bold text-xs sm:text-sm ${rankTab === "CLASS" ? "bg-yellow-300 text-black font-black" : "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-400"}`}
            >
              {className}반 랭킹
            </button>
          </div>
              ...
              <table className="w-full text-left font-retro text-[10px] sm:text-xs border-collapse">
                <thead>
                  <tr className="border-b-2 border-slate-700 text-yellow-400">
                    ...
                    <th className="py-2 px-1">플레이어</th>
                    <th className="py-2 px-1 text-right">점수</th>
                  </tr>
                </thead>
                <tbody>
                  {rankingList.map((record, index) => {
                    const isCurrentUser = record.class_name === className && record.number === studentNumber && record.score === score;
                    return (
                      <tr key={index} className={`border-b border-slate-800 hover:bg-slate-900 ${isCurrentUser ? "bg-cyan-950/70 text-cyan-300 font-bold" : "text-white"}`}>
                        <td className="py-2 px-1">{index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `${index + 1}`}</td>
                        <td className="py-2 px-1">{record.class_name}#{String(record.number).padStart(2, "0")}</td>
